...
class ScanNetDataset(utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_name0 = osp.join(self.root_dir, scene_name,
                             "color", f"{stem_name_0}.jpg")
        img_name1 = osp.join(self.root_dir, scene_name,
                             "color", f"{stem_name_1}.jpg")

        # TODO: Support augmentation & handle seeds for each worker correctly.

        image0 = read_scannet_color(img_name0, resize=(640, 480))
        image1 = read_scannet_color(img_name1, resize=(640, 480))
        image0_raw = read_scannet_rgb_raw(img_name0, resize=(640, 480))
        image1_raw = read_scannet_rgb_raw(img_name1, resize=(640, 480))
        # read the depthmap which is stored as (480, 640)
        if self.mode in ["train", "val"]:
            depth0 = read_scannet_depth(
                osp.join(self.root_dir, scene_name,
                         "depth", f"{stem_name_0}.png")
            )
            depth1 = read_scannet_depth(
                osp.join(self.root_dir, scene_name,
                         "depth", f"{stem_name_1}.png")
            )
        else:
            depth0 = read_scannet_depth(
                osp.join(self.root_dir, scene_name,
