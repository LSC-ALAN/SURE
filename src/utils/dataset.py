...
def read_scannet_rgb_raw(path, resize=(640, 480), augment_fn=None):
    """
    Args:
        path (str): image path
        resize (tuple): resize to (w, h)
        augment_fn (callable, optional): optional augmentation function
    Returns:
        image (torch.tensor): (3, h, w), uint8
    """
    # read image using cv2 (BGR), then convert to RGB
