...
def unnormalize(img_tensor):
    """
    反归一化张量图像 (CHW Tensor)，返回 numpy 图像 (HWC) 范围 0~255。
    """
...
def _make_evaluation_figure(data, b_id, alpha="dynamic",dpi=150):
    ...
    #         * 255).round().astype(np.int32)
    # img0 = (data["image0"][b_id].permute(1, 2, 0).cpu().numpy() * 255).round().astype(np.uint8)
    # img1 = (data["image1"][b_id].permute(1, 2, 0).cpu().numpy() * 255).round().astype(np.uint8)
    img0 = unnormalize(data["image0"][b_id])
    img1 = unnormalize(data["image1"][b_id])
    ...
    # make the figure
    figure = make_matching_figure(img0, img1, kpts0, kpts1, color, text=text,dpi=dpi)
    return figure
