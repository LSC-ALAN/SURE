...
def aggregate_metrics(metrics, epi_err_thr=5e-4, config=None):
    ...
    if config.SURE.EVAL_TIMES >= 1:
        pose_errors = (
            ...
            .reshape(-1, config.SURE.EVAL_TIMES)[unq_ids]
            .reshape(-1)
        )
    else:
    ...
    corr_epistemic_mean = np.mean([metrics["corr_epistemic"][i] for i in unq_ids])
    corr_aleatoric_mean = np.mean([metrics["corr_aleatoric"][i] for i in unq_ids])
    corr_alea_epi_mean = np.mean([metrics["corr_alea_epi"][i] for i in unq_ids])
    return {**aucs, **precs, **num_matches,"corr_alea_epi": corr_epistemic_mean,
        "corr_aleatoric_mean": corr_aleatoric_mean,"corr_alea_epi_mean":corr_alea_epi_mean}
